...
| A6 | 2026-09-25 | **Phase 10 — email and password sign-in alongside Google.** (a) **Register** page: name, email, password, confirm password; the account is created and signed in immediately; no email verification. Emails are trimmed and lower-cased. (b) **Password rules:** 8–128 characters, no composition rules (NIST SP 800-63B), confirmation must match; hashed with `scrypt` from `node:crypto` (per-user random salt, constant-time comparison, no new dependency); the hash is never logged or returned. (c) **Sign in** with email and password on a new sign-in page that also offers Google; one generic error ("email or password is incorrect") for unknown email, wrong password or a Google-only account; failed attempts are rate-limited per client IP and email. (d) **Email already used by a Google-only account:** registration does not create or change an account; it tells the person to sign in with Google and set a password in **Account**. The Account page (signed in) lets a user without a password set one, and a user with a password change it (current password required). After that both sign-in methods open the same account. (e) **Password account, then Google with the same email:** Google proves ownership of the email (`email_verified`), so the Google account is linked to the existing user and the existing password is **cleared**, because it was set without email verification (protects against account pre-hijacking); the person can set a new password in Account. (f) **Sessions switch from database to JWT** (Auth.js requires JWT for credentials sign-in); users and accounts stay in PostgreSQL; existing sessions end once at deploy; signing out clears the session cookie. (g) All new screens in en/fr/pt-BR, WCAG 2.2 AA, both themes. **Out of scope:** password reset and email verification (both need email sending), declared in the README | Human request before submitting: the evaluator can sign in without being added as a Google test user, and one person can use both methods. Decisions (d) link only after proof of email ownership, (e) clear the unverified password on Google link, (f) JWT sessions, (b)/(c) lean password rules without recovery — approved by the human on 2026-09-25 |
